...
项目使用 **Yarn 3.2.3 Workspaces + Lerna**（独立版本号）管理，共 11 个包：
...
**关键源文件**：`packages/http-server/src/manager.ts`、`packages/cli/src/core/manager-init.ts`
...
**关键源文件**：`packages/http-server/src/index.ts`、`packages/http-server/src/routes/`
...
**REST API 路由**（`packages/http-server/src/routes/index.ts`）：
...
**SSE 事件推送**（`packages/http-server/src/routes/event.ts`）：
...
**关键源文件**：`packages/cli/src/bin.ts`、`packages/cli/src/core/manager-init.ts`
...
**CLI 命令列表**（`packages/cli/src/bin.ts`）：
...
**关键源文件**：`packages/electron/src/index.ts`
