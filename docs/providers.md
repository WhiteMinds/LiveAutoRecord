...
| `authFields`                | —                      | （可选）声明鉴权配置字段（如 cookie）              |
| `authFlow`                  | —                      | （可选）声明浏览器自动登录流程                     |
| `setAuth`                   | 加载配置时、用户设置时 | （可选）注入鉴权配置到 API 层                      |
| `checkAuth`                 | 检查鉴权状态时         | （可选）验证当前鉴权是否有效                       |
...
import { Recorder, RecorderCreateOpts, defaultToJSON, genRecorderUUID } from '@autorecord/manager'
 ...
 */
function createTimeoutChecker(onTimeout: () => void, time: number): { update: () => void; stop: () => void } {
...
文件：`apps/http-server/src/manager.ts`
...
文件：`apps/cli/src/core/manager-init.ts`
...
确保 `pnpm-workspace.yaml` 中包含新包路径（`packages/*` 通配符通常已覆盖）：
...
如果你的平台需要登录才能获取高画质直播流或避免频率限制，可以实现鉴权接口。鉴权配置在 Provider 级别，所有同平台的录制器共享同一份凭据。
...
### 5.2 实现示例（B站 Cookie 鉴权）

以 B站为例，完整鉴权实现分为两部分：API 层的 Cookie 注入和 Provider 的鉴权接口。

**API 层 Cookie 注入**（`packages/bilibili-recorder/src/bilibili_api.ts`）：
...
// Axios 请求拦截器：自动在所有 API 请求中注入 Cookie
...
export async function getNavInfo(): Promise<{
  ...
  const res = await requester.get('https://api.bilibili.com/x/web-interface/nav')
  return res.data.data
}
```

**Provider 鉴权接口**（`packages/bilibili-recorder/src/index.ts`）：
...
**FFmpeg 录制时注入 Cookie**（在 `checkLiveStatusAndRecord` 中）：
...
const command = createFFMPEGBuilder()
  .input(stream.url)
  .addInputOptions('-user_agent', 'Mozilla/5.0 ...', '-headers', headersValue)
```

### 5.3 鉴权配置的生命周期

1. **加载**：应用启动时，`initRecorderManager()` 从 `manager.json` 的 `providerAuthConfigs` 读取配置，调用 `provider.setAuth()` 注入
2. **设置**：用户通过 Web UI / CLI / API 设置鉴权，调用 `provider.setAuth()` 并持久化到 `manager.json`
3. **验证**：调用 `provider.checkAuth()` 验证凭据是否有效（如调用 B站 nav API）
4. **使用**：Provider 内部的 API 请求和 FFmpeg 命令自动携带凭据
5. **清除**：用户清除鉴权时，调用 `provider.setAuth({})` 并从持久化存储中删除

### 5.4 注意事项

- `authFields` / `authFlow` / `setAuth` / `checkAuth` 均为可选，不需要鉴权的 Provider 完全不用实现
- `authFlow.checkLoginResult` 是纯函数，不应有副作用，由外部执行器（Playwright / BrowserWindow）每秒调用
- Cookie 注入需要同时覆盖 API 请求（Axios 拦截器）和 FFmpeg 下载（`-headers` 参数）
- 鉴权配置的 key 名需要与 `authFields` 中的 `key` 对应
...
```bash
# 全量构建（Turborepo 自动拓扑排序）
pnpm build

# 通过 CLI 快速验证
node apps/cli/lib/bin.js resolve https://www.myplatform.com/123456
node apps/cli/lib/bin.js add https://www.myplatform.com/123456
node apps/cli/lib/bin.js check 1
```
...
| `apps/http-server/src/manager.ts`                      | HTTP Server 侧 provider 注册                                                            |
| `apps/cli/src/core/manager-init.ts`                    | CLI 侧 provider 注册                                                                    |
